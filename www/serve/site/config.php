 ...
 *
 */
$config->prependTemplateFile = '_INIT_.php';
 ...
 *
 */
$config->appendTemplateFile = '_OUT_.php';
 ...
 *
 * Set to false to disable query caching. This will make everything run slower so should 
...
if (preg_match( '/circles\.jejotte\.com/', $_SERVER['HTTP_HOST']) ) {
	...
	$config->dbPass = '2d0am4oa72.e0s8c_2dc0';
	$config->dbPort = '3306';
	// Installer: HTTP Hosts Whitelist
	$config->httpHosts = array('circles.jejotte.com', 'www.circles.jejotte.com');

} elseif (preg_match( '/circles\.dev/', $_SERVER['HTTP_HOST'])
		|| ($_SERVER['HTTP_HOST']===$_SERVER['SERVER_ADDR']))
{	
	// dev
	$config->dbHost = 'localhost';
	$config->dbName = 'circles';
	$config->dbUser = 'circles_user';
	$config->dbPass = '2d0am4oa72.e0s8c_2dc0';
	$config->dbPort = '3306';
	// Installer: HTTP Hosts Whitelist
	$config->httpHosts = array('circles.dev', 'www.circles.dev', $_SERVER['SERVER_ADDR'] );
} else {
	die('No known config.');
}

$config->set('site_date_format', 'Y-m-d');
 ...
 * 
 */
$config->userAuthSalt = '80792a1ac0681d206d8db7377ab60bb4'; 
...
define( 'TUMBLR_API_KEY', 'HXeiShIu8RdDnkDprTOUB591OtYfgnauldSujgHP3WiHfowwxK');
define( 'TUMBLR_SECRET', 'MDtAROGkUbMEwLPmM9Oo7dlwa9IFPmP6Bif9yfF5kKBxJxWTUy');
...
define( 'ICON_NUMBER_OF_PEOPLE', 'fa-spinner' );
define( 'ICON_AMENITIES', 'fa-toggle-on');
define( 'ICON_NEIGHBOURHOODS', 'glyphicon-map-marker');
 ...
 * 
 */
$config->userAuthSalt = '7edd4f9b2d3a45dad69f9f3f1ecb870c'; 
 ...
 * 
 */
$config->httpHosts = array('circles.dev', 'www.circles.dev');
